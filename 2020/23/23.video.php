	...
	foreach($rapord as $ord)
	{
		foreach($grunnordpoeng as $gord => $poeng)
		{
			...
			if($pos !== false)
			{
				...
				if($sanger == 'Nizzy G')
				{
					$im = imagecreatefrompng("h-alv.png");
					$nizzord = $ordverdi+$linjeverdi;
				}
				else
				{
					$im = imagecreatefrompng("v-alv.png");
					$lilord = $ordverdi+$linjeverdi;
				}
				$black = imagecolorallocate($im, 0, 0, 0);
				imagettftext($im, 16, 0, 215, 115, $black, $font, $ord);
				imagettftext($im, 16, 0, 50, 430, $black, $font, ($totalsum['Lil Niz X']+$lilord));
				imagettftext($im, 16, 0, 50, 460, $black, $font, 'Lil Niz X');
				imagettftext($im, 16, 0, 330, 430, $black, $font, ($totalsum['Nizzy G']+$nizzord));
				imagettftext($im, 16, 0, 330, 460, $black, $font, 'Nizzy G');
				
				for($i = 0; $i <= 12; $i++)
				{
					imagepng($im, "imgs/" . str_pad($imgno, 5, "0", STR_PAD_LEFT) . ".png");
					$imgno++;
				}
				break;
			}
